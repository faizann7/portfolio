...
export default function PasswordGate({ children, projectId = "", isInline = false }: PasswordGateProps) {
    ...
    const LoginForm = (isCompact: boolean = false) => (
        <div className={isCompact ? "" : "w-full max-w-md"}>
            <div className="text-center mb-10">
                ...
                <h1 className={`${isCompact ? 'text-2xl' : 'text-3xl'} font-bold mb-3 font-heading tracking-tight`}>
                    Confidential Process
                </h1>
                <p className="text-white/50 text-lg leading-relaxed mb-6">
                    Enter password to unlock the full case study.
                </p>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                >
                    <input
                        ...
                        className={`w-full bg-white/5 border ${error ? 'border-red-500/50' : 'border-white/10'} rounded-[var(--radius-card-inner)] px-6 py-4 pr-14 outline-none focus:border-white/30 focus:bg-white/[0.07] transition-all text-center text-xl placeholder:text-white/20`}
                        autoFocus={!isInline}
                    />
                    <button
                        type="button"
                ...
                <button
                    type="submit"
                    className="w-full bg-white text-black font-semibold py-4 rounded-[var(--radius-card-inner)] hover:bg-opacity-90 active:scale-[0.98] transition-all text-lg shadow-lg shadow-white/5 cursor-pointer"
                >
                    Unlock Case Study
                </button>
            </form>
        ...
        return (
            <div className="relative w-full py-20 flex items-center justify-center">
                <motion.div
                    ...
                    className="w-full max-w-md bg-white/[0.03] backdrop-blur-xl border border-white/10 rounded-[var(--radius-card)] p-[var(--card-padding)] md:p-[var(--card-padding-lg)] shadow-2xl z-20"
                >
                    {LoginForm(true)}
                </motion.div>
            ...
            <div className="min-h-[60vh] flex items-center justify-center px-4 relative mb-24">
                ...
                <div className="w-full max-w-md relative z-10">
                    <motion.div
                        ...
                        animate={{ opacity: 1, y: 0 }}
                        transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1] }}
                        className="bg-white/[0.03] backdrop-blur-xl border border-white/10 rounded-[var(--radius-card)] p-[var(--card-padding)] md:p-[var(--card-padding-lg)] shadow-2xl"
                    >
                        {LoginForm(false)}
                    </motion.div>
                </div>
